...
- **Add-on interface:** The add-on follows patterns from [ESPHome](https://github.com/esphome/esphome) and [Matterbridge](https://github.com/Luligu/matterbridge-home-assistant-addon). The Web UI embeds the integration panel in a full-viewport iframe (no redirect), inspired by [Matterbridge Roborock plugin](https://github.com/RinDevJunior/matterbridge-roborock-vacuum-plugin)-style UIs; `panel_title`, `panel_icon`, `ingress_port` for the config server, and a `schema` + `options` so users get a proper **Configuration** tab (e.g. “Reinstall integration on start”). Translations in `translations/en.yaml` give friendly labels for schema options.
